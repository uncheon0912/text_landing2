...
function wireLeadLinks() { document.querySelectorAll('a').forEach(link => { const label=String(link.textContent||'').trim(); if (link.getAttribute('href') === 'index.html#contact' || /상담 예약하기|문의하기|견적문의|고객지원/.test(label)) link.setAttribute('href','lead.html'); }); }
